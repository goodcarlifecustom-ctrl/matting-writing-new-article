...
import { argvValue, DEFAULT_CATEGORY, loadInput, normalizeRelatedKeywords, normalizeSpaces, parseList, parseScalar, postToWpFromInputs, resolveTargetMedia, slugFromKeyword, yamlListEntry, yamlString } from './workflow-utils.mjs';
...
const wordpressDraft = argvValue(process.argv, 'wordpress_draft') ?? argvValue(process.argv, 'wordpress-draft') ?? parseScalar(inputText, 'wordpress_draft') ?? 'false';
let postToWp;
...
const targetMedia = resolveTargetMedia(valueFrom(inputText, 'target_media'));
...
const generatedSlug = slugFromKeyword(mainKeyword);
const slug = providedSlug || generatedSlug;
if (!slug) { console.error('Could not generate a meaningful slug. Provide --slug after keyword analysis.'); process.exit(1); }
...
const tags = parseList(inputText, 'tags');
...
const citationSources = parseList(inputText, 'citation_sources');
...
const inputYml = [
  ...
  yamlListEntry('related_keywords', relatedKeywords),
  ...
  yamlListEntry('tags', tags),
  `target_media: ${targetMedia === null ? 'null' : yamlString(targetMedia)}`,
  `render_profile: ${renderProfile}`,
  yamlListEntry('internal_link_candidates', internalLinkCandidates),
  yamlListEntry('reference_urls', referenceUrls),
  yamlListEntry('citation_sources', citationSources),
...
const metadata = { title: title === 'auto' ? null : title, slug, canonical_slug: slug, meta_description: null, target_keyword: mainKeyword, related_keywords: relatedKeywords, search_intent: null, persona, article_type: articleType, article_purpose: articlePurpose, min_char_count: minWordCount, target_char_count: targetWordCount, max_char_count: maxWordCount, min_word_count: minWordCount, target_word_count: targetWordCount, max_word_count: maxWordCount, status: 'draft', render_profile: renderProfile, wordpress_draft: false, post_to_wp: false, wordpress_status: 'DISABLED', delivery_mode: 'manual_copy', primary_output: 'article-decorated.html', copy_ready: false, external_write_performed: false, category, tags, target_media: targetMedia, internal_link_candidates: internalLinkCandidates, citation_source_candidates: citationSources, content_status: 'PENDING', review_evidence_status: 'PENDING', source_verification_status: 'PENDING', source_policy_status: 'PENDING', decoration_status: 'PENDING', draft_readiness: 'NOT_READY', publish_readiness: 'NOT_READY', created_at: now, updated_at: now, research_date: null, notes };
await writeFile(path.join(dir, 'metadata.json'), JSON.stringify(metadata, null, 2) + '\n', 'utf8');
await writeFile(path.join(dir, 'source-manifest.json'), JSON.stringify({ version: SOURCE_POLICY.manifest_version, sources: [] }, null, 2) + '\n', 'utf8');
await writeFile(path.join(dir, 'section-evidence.json'), JSON.stringify({ version: 1, article_slug: slug, sections: [], evidence_items: [] }, null, 2) + '\n', 'utf8');
...
for (const file of ['serp.md','headings.csv','heading-analysis.md','heading-plan.md','draft.md','article.html','article-linked.html','article-decorated.html','external-links.md','check-report.md']) await writeFile(path.join(dir, file), '', 'utf8');
